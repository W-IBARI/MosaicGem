...
- `rate` 为百分比（0-100）
- 孔数上限为**三层**，对具体物品按优先级取用（任一不满足都会被拦截并提示）：
  - **物品 id 上限**（覆盖类型与全局）：`settings.max-holes-by-id` 按**材质大写名**（如 `IRON_SWORD`、`SHIELD`）配置，命中即生效
  - **物品类型上限**（覆盖全局）：`settings.max-holes-by-type` 按**物品类型**（与打孔器 `targetType` 同语义，如 `SWORD`/`HELMET`/`CROSSBOW`，大小写不敏感）配置，未命中 id 层时生效
  - **全局上限**（兜底）：装备所有来源的孔数之和不得超过 `settings.max-holes`；值 `0` 表示该类型/物品不可打孔
...
settings:
  max-holes: 6            # ① 全局（兜底）
  max-holes-by-type:      # ② 物品类型（覆盖①）
    SWORD: 8
    HELMET: 4
  max-holes-by-id:        # ③ 物品 id（材质大写名，覆盖①②）
    IRON_SWORD: 10
    SHIELD: 1
```

实际取值顺序：`③ 物品id` → `② 物品类型` → `① 全局`；未配置的键落在全局值。
